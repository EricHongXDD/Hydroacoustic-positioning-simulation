...
class AppWidget(QMainWindow, Ui_MainWindow):
    ...
    def search_info_from_files(self):
        ...
        for temp in search_type_map:
            if search_type == temp[0]:
                ...
                self.cursor.execute(sql)
                i = 0
                while self.cursor.fetchone():  # 每次查询一行记录
                    for j in range(1, 19):
    ...
    def load_all_infos(self):
        ...
        self.cursor.execute(sql)
        self.cursor.fetchone()
        self.cursor.execute('select count(*) from person')  # 执行查询
        result = self.cursor.fetchone()  # 获取结果
        if result:
            count = result[0]  # 如果有结果，则取得计数
            self.tableWidget_01_run_info.setRowCount(count)  # 设置行数
        else:
            # 处理没有结果的情况，可能是表为空或者查询语句有误
            print("查询没有返回结果。")
            # 适当的处理方式，比如可以设置行数为0或者显示一个错误信息
